...
for (traitName in trait_lst) {
  ...
  # res.df = res.df[which.max(res.df$Estimate),]
  
  thres = 0.2
  for (thres in seq(0.1,0.9,by=0.01)) {
    if (isTRUE(all.equal(thres %% 0.05, 0))) {
      cat(traitName, " -- thres: ", thres, "\n")
    }
    common$gwas = NA
    fm.sub = subset(fm,PIP.L_10 > thres | PIP.L_1 > thres)
    ...
    i = which(cell_lst==cell & dataset_lst==dataset)
    ...
    if (sum(common$gwas[idx],na.rm=T)>1) {
      ...
      j = j + 1
      res.lst[[j]] = data.frame(trait=traitName,dataset,cell,thres,Estimate=res["Estimate"],se = res["Std. Error"],tvalue=res["t value"],pval=res["Pr(>|t|)"])
    }
  }
...
f.out = paste0("/oak/stanford/groups/smontgom/amarder/chrombpnet_variant_effects/output/data/cbp/analysis/ukb_fm_gwas_binarizedpip_enrichments.meta_best_celltype_continuous_predictions.txt")
...
for (organ in c("heart_artery","brain")) {
  ...
  f = paste0("/oak/stanford/groups/smontgom/amarder/chrombpnet_variant_effects/output/data/cbp/analysis/ukb_fm_gwas_binarizedpip_enrichments.qtl.",organ,".txt")
  res.df = fread(f,data.table = F,stringsAsFactors = F)
  res.df = res.df[which.min(res.df$pval),]
  ...
  for (thres in seq(0.1,0.9,by=0.01)) {
    if (isTRUE(all.equal(thres %% 0.05, 0))) {
      cat(organ, " -- thres: ", thres, "\n")
    }
    
    common$gwas = NA
    finemap.sub = subset(finemap,V6 > thres)
    ...
    if (sum(common$gwas[idx],na.rm=T)>1) {
      ...
      res = summary(mod)$coef[2,]
      j = j + 1
      res.lst[[j]] = data.frame(trait=organ,dataset,cell,thres,Estimate=res["Estimate"],se = res["Std. Error"],tvalue=res["t value"],pval=res["Pr(>|t|)"])
    }
  }
}
...
f.out = paste0("/oak/stanford/groups/smontgom/amarder/chrombpnet_variant_effects/output/data/cbp/analysis/ukb_fm_gwas_binarizedpip_enrichments.qtl.","best_celltype_continuous_predictions",".txt")
fwrite(res.df,f.out,quote = F,na = "NA",sep = '\t',row.names = F,col.names = T)
...
for (traitName in trait_lst) {
  ...
  for (thres in seq(0.1,0.9,by=0.01)) {
    ...
    common$gwas = NA
    fm.sub = subset(finemap.ukb.sub,pip > thres)
...
f.out = paste0("/oak/stanford/groups/smontgom/amarder/chrombpnet_variant_effects/output/data/cbp/analysis/ukb_fm_gwas_binarizedpip_enrichments.","best_celltype_continuous_predictions",".txt")
fwrite(res.df,f.out,quote = F,na = "NA",sep = '\t',row.names = F,col.names = T)
