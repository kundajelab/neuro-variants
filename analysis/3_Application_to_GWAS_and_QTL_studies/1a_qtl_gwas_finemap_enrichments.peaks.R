...
for (traitName in trait_lst) {
  ...
  for (i in 1:length(cell_lst)) {
    ...
    idx = !is.na(common$gwas)
    # test predicting fine map variants
    if (sum(common$gwas[idx],na.rm=T)>1) {
      mod = glm(common[idx,paste0("peak_overlap.",cell,".",dataset)]~common$gwas[idx] + log10(common$gene_distance_1[idx]+1) + common$s_het_1[idx],family = binomial(link="logit"))
      res = summary(mod)$coef[2,]
      j = j + 1
      res.lst[[j]] = data.frame(trait=traitName,dataset,cell,Estimate=res["Estimate"],se = res["Std. Error"],tvalue=res["z value"],pval=res["Pr(>|z|)"])
    }
  }
  ...
  f.out = paste0("/oak/stanford/groups/smontgom/amarder/chrombpnet_variant_effects/output/data/cbp/analysis/ukb_fm_gwas_binarizedpip_enrichments.peaks.",traitName,".txt")
...
for (organ in c("heart_artery","brain")) {
  ...
  for (i in 1:length(cell_lst)) {
    ...
    if (sum(common$gwas[idx],na.rm=T)>1) {
      ...
      res = summary(mod)$coef[2,]
      j = j + 1
      res.lst[[j]] = data.frame(trait=organ,dataset,cell,Estimate=res["Estimate"],se = res["Std. Error"],tvalue=res["z value"],pval=res["Pr(>|z|)"])
    }
  }
  ...
  f.out = paste0("/oak/stanford/groups/smontgom/amarder/chrombpnet_variant_effects/output/data/cbp/analysis/ukb_fm_gwas_binarizedpip_enrichments.peaks.qtl.",organ,".txt")
  fwrite(res.df,f.out,quote = F,na = "NA",sep = '\t',row.names = F,col.names = T)
