...
for (thres in c(0.005,0.05,0.1)) {
  print(thres)
  
  keep = adult_brain
  df$num_peaks_ab = apply(df[,paste0("peak_overlap.",keep)],1,sum,na.rm=T)
  df$num_cbp_ab = apply(df[,paste0("abs_logfc.mean.pval.",keep)],1,function(x) {sum(x<thres,na.rm = T)})
  tmp = df[,paste0("abs_logfc.mean.pval.",keep)]
  for (i in 1:length(keep)) {tmp[,i][!df[,paste0("peak_overlap.",keep[i])]] = 1}
  df$num_peakscbp_ab = apply(tmp,1,function(x) {sum(x<thres,na.rm = T)})
  
  keep = fetal_brain
  df$num_peaks_fb = apply(df[,paste0("peak_overlap.",keep)],1,sum,na.rm=T)
  df$num_cbp_fb = apply(df[,paste0("abs_logfc.mean.pval.",keep)],1,function(x) {sum(x<thres,na.rm = T)})
  tmp = df[,paste0("abs_logfc.mean.pval.",keep)]
  for (i in 1:length(keep)) {tmp[,i][!df[,paste0("peak_overlap.",keep[i])]] = 1}
  df$num_peakscbp_fb = apply(tmp,1,function(x) {sum(x<thres,na.rm = T)})
  
  keep = fetal_heart
  df$num_peaks_fh = apply(df[,paste0("peak_overlap.",keep)],1,sum,na.rm=T)
  df$num_cbp_fh = apply(df[,paste0("abs_logfc.mean.pval.",keep)],1,function(x) {sum(x<thres,na.rm = T)})
  tmp = df[,paste0("abs_logfc.mean.pval.",keep)]
  for (i in 1:length(keep)) {tmp[,i][!df[,paste0("peak_overlap.",keep[i])]] = 1}
  df$num_peakscbp_fh = apply(tmp,1,function(x) {sum(x<thres,na.rm = T)})
  
  keep = adult_heart
  df$num_peaks_ah = apply(df[,paste0("peak_overlap.",keep)],1,sum,na.rm=T)
  df$num_cbp_ah = apply(df[,paste0("abs_logfc.mean.pval.",keep)],1,function(x) {sum(x<thres,na.rm = T)})
  tmp = df[,paste0("abs_logfc.mean.pval.",keep)]
  for (i in 1:length(keep)) {tmp[,i][!df[,paste0("peak_overlap.",keep[i])]] = 1}
  df$num_peakscbp_ah = apply(tmp,1,function(x) {sum(x<thres,na.rm = T)})
  ...
  # save results:
  f.out = paste0("/oak/stanford/groups/smontgom/amarder/chrombpnet_variant_effects/output/data/FinalAnalysis/rare_vs_common.celltype_specificity.",thres,".txt")
