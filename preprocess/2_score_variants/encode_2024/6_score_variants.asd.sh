...
log_dir=/oak/stanford/groups/akundaje/projects/neuro-variants/logs/variant_scores/asd/encode_2024/K562_bias
...
for clust in $model_dir/*; do
    ...
    for fld in {0..4}; do
        ...
        sbatch -J $cluster.$fold.asd -t $time -c $cpus --mem=$mem \
            ...
            $jobscript $scoring_script \
                ...
                -pg $hg38_ref_fasta \
                -ps $hg38_chrom_sizes \
                -o $score_dir/$cluster/$fold/$cluster.$fold.asd \
                -t 1000000 \
                -sc chrombpnet \
                --no_hdf5
    done
