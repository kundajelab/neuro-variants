...
peaks_dir=/oak/stanford/groups/akundaje/projects/neuro-variants/chrombpnet_inputs/encode_2024/peaks/filtered
score_dir=/oak/stanford/groups/akundaje/projects/neuro-variants/variant_scores/rare/encode_2024/K562_bias
log_dir=/oak/stanford/groups/akundaje/projects/neuro-variants/logs/variant_scores/rare/encode_2024/K562_bias
...
# for clust in $model_dir/*.H; do

for clust in $model_dir/*cardiomyocyte.I*; do
    ...
    for fld in {0..4}; do
        ...
        if [[ -n $(ls -A "$score_dir/$cluster/$fold" 2>/dev/null) && $(ls "$score_dir/$cluster/$fold"/*.variant_scores.tsv 2>/dev/null) ]]; then
        ...
        sbatch -J $cluster.$fold.rare -t $time -c $cpus --mem=$mem \
            ...
            $jobscript $scoring_script \
                ...
                -pg $hg38_ref_fasta \
                -ps $hg38_chrom_sizes \
                -o $score_dir/$cluster/$fold/$cluster.$fold.rare \
                -t 1000000 \
                -sc chrombpnet \
                --no_hdf5
    done
