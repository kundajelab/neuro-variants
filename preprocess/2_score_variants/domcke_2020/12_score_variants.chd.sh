...
log_dir=/oak/stanford/groups/akundaje/projects/neuro-variants/logs/variant_scores/chd/domcke_2020/K562_bias
...
for clust in $model_dir/*; do
    ...
    for fld in {0..4}; do
        ...
        sbatch -J $cluster.$fold.chd -t $time -c $cpus --mem=$mem \
            ...
            $jobscript $scoring_script \
                ...
                -pg $hg19_ref_fasta \
                -ps $hg19_chrom_sizes \
                -o $score_dir/$cluster/$fold/$cluster.$fold.chd \
                -t 1000000 \
                -sc chrombpnet \
                --no_hdf5
    done
