...
variants=/oak/stanford/groups/akundaje/projects/neuro-variants/variant_lists/shap_variants.rosmap.model_inputs.hg38.tsv
model_dir=/oak/stanford/groups/akundaje/projects/neuro-variants/chrombpnet_models/corces_2020/K562_bias
shap_dir=/oak/stanford/groups/akundaje/projects/neuro-variants/variant_shap/rosmap/K562_bias/corces_2020
log_dir=/oak/stanford/groups/akundaje/projects/neuro-variants/logs/variant_shap/rosmap/K562_bias/corces_2020
...
# for cluster in corces_2020.Cluster1 corces_2020.Cluster8 corces_2020.Cluster11 corces_2020.Cluster13 corces_2020.Cluster19 corces_2020.Cluster22 corces_2020.Cluster24; do
...
for clust in $model_dir/*; do
    ...
    for fld in {0..4}; do
        ...
        score_output_file=$shap_dir/$cluster/$fold/$cluster.$fold.rosmap.shap.variant_scores.tsv
        shap_output_file=$shap_dir/$cluster/$fold/$cluster.$fold.rosmap.shap.variant_shap.counts.h5
        ...
        sbatch -J $cluster.$fold.rosmap.shap.variant_scores -t $time -c $cpus --mem=$mem \
            ...
            $jobscript $scoring_script \
                ...
                -o $shap_dir/$cluster/$fold/$cluster.$fold.rosmap.shap \
                -t 0 \
                -sc chrombpnet

        [[ -f $shap_output_file ]] || \
        sbatch -J $cluster.$fold.rosmap.shap.variant_shap -t $time -c $cpus --mem=$mem \
            -p $partitions --gpus 1 --requeue \
